...
type RateLimitConfig struct {
	UserBytesPerSec       int64                         // 每用户每秒下载字节数限制 (默认: 10MB/s)
...
func NewRateLimitConfig() RateLimitConfig {
	return RateLimitConfig{
		UserBytesPerSec:       conf.GetConf().System.UserBytesLimitPerSec,
...
func NewRateLimitedWriter(ctx context.Context, writer io.Writer, config RateLimitConfig, fileSize int64, rangeInfo *RangeInfo) *RateLimitedWriter {
	...
	// 添加调试日志
	fmt.Printf("[RateLimit] 初始化 - 文件大小: %.2fMB, 传输大小: %.2fMB, 初始速率: %.2fMB/s, Burst: %.2fMB\n",
		float64(fileSize)/float64(MB),
		float64(actualTransferSize)/float64(MB),
		float64(initialRate)/float64(MB),
		float64(userBurstSize)/float64(MB))
...
// Write 实现io.Writer接口，支持动态速率调整和大数据块分批处理
...
func (w *RateLimitedWriter) adjustRateIfNeeded() {
	...
	// 计算当前平均速度
	currentSpeed := int64(float64(w.downloadedBytes) / elapsed)
	...
	if rateDiff > 0.1 || rateDiff < -0.1 {
		...
		// 每30秒记录一次调整信息
		if int(elapsed)%30 == 0 {
			fmt.Printf("[RateLimit] VIP动态调整: VIP%d, %.1fs, 进度: %.1f%%, %d → %d KB/s, 当前实际: %.1f KB/s\n",
				w.config.VipLevel, elapsed, progress*100,
				currentLimit/1024, adjustedRate/1024, float64(currentSpeed)/1024)
		}
	}
}
